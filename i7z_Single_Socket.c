...
void print_i7z_socket_single(
    struct cpu_socket_info socket_0, int printw_offset, int PLATFORM_INFO_MSR,
    int PLATFORM_INFO_MSR_high, int PLATFORM_INFO_MSR_low, int *online_cpus,
    double cpu_freq_cpuinfo, struct timespec one_second_sleep, char TURBO_MODE,
    char *HT_ON_str, int *kk_1, U_L_L_I *old_val_CORE, U_L_L_I *old_val_REF,
    U_L_L_I *old_val_C3, U_L_L_I *old_val_C6, U_L_L_I *old_val_C7,
    U_L_L_I *old_TSC, int estimated_mhz, U_L_L_I *new_val_CORE,
    U_L_L_I *new_val_REF, U_L_L_I *new_val_C3, U_L_L_I *new_val_C6,
    U_L_L_I *new_val_C7, U_L_L_I *new_TSC, double *_FREQ, double *_MULT,
    long double *C0_time, long double *C1_time, long double *C3_time,
    long double *C6_time, long double *C7_time, struct timeval *tvstart,
    struct timeval *tvstop, int *max_observed_cpu);
...
void print_i7z_socket_single(
  ...
  printf("cpuinfo might be wrong if cpufreq is enabled. To guess correctly try "
         "estimating via tsc\n");
  ...
  unsigned long long int CPU_CLK_UNHALTED_CORE, CPU_CLK_UNHALTED_REF,
      CPU_CLK_C3, CPU_CLK_C6, CPU_CLK_C1, CPU_CLK_C7;
  // current blck value
  float BLCK;
  ...
  *max_observed_cpu = (socket_0.max_cpu > *max_observed_cpu)
                          ? socket_0.max_cpu
                          : *max_observed_cpu;

  int core_list_size_phy, core_list_size_log;
  if (socket_0.max_cpu > 0) {
    // set the variable print_core to 0, use it to check if a core is online and
    // doesnt have any garbage values
    memset(print_core, 0, 6 * sizeof(char));

    // We just need one CPU (we use Core-1) to figure out the multiplier and the
    // bus clock freq. multiplier doesnt automatically include turbo note turbo
    // is not guaranteed, only promised So this msr will only reflect the actual
    // multiplier, rest has to be figured out
    ...
    /*if (CPU_NUM == -1)
    {
          sleep (1);		//sleep for a bit hoping that the offline socket
    becomes online continue;
    }*/
    ...
    // 3B defines till Max 4 Core and the rest bit values from 32:63 were
    // reserved.
    int MAX_TURBO_1C = 0, MAX_TURBO_2C = 0, MAX_TURBO_3C = 0, MAX_TURBO_4C = 0,
        MAX_TURBO_5C = 0, MAX_TURBO_6C = 0;
    ...
    if (E7_mp_present) {
      ...
      // Bits:0-7  - core1
      MAX_TURBO_1C =
          get_msr_value(CPU_NUM, MSR_TURBO_RATIO_LIMIT, 7, 0, &error_indx);
      SET_IF_TRUE(error_indx, online_cpus[0], -1);
      // Bits:15-8 - core2
      MAX_TURBO_2C =
          get_msr_value(CPU_NUM, MSR_TURBO_RATIO_LIMIT, 15, 8, &error_indx);
      SET_IF_TRUE(error_indx, online_cpus[0], -1);
      // Bits:23-16 - core3
      MAX_TURBO_3C =
          get_msr_value(CPU_NUM, MSR_TURBO_RATIO_LIMIT, 23, 16, &error_indx);
      SET_IF_TRUE(error_indx, online_cpus[0], -1);
      // Bits:31-24 - core4
      MAX_TURBO_4C =
          get_msr_value(CPU_NUM, MSR_TURBO_RATIO_LIMIT, 31, 24, &error_indx);
      SET_IF_TRUE(error_indx, online_cpus[0], -1);
      ...
      // Bits:39-32 - core4
      MAX_TURBO_5C =
          get_msr_value(CPU_NUM, MSR_TURBO_RATIO_LIMIT, 39, 32, &error_indx);
      SET_IF_TRUE(error_indx, online_cpus[0], -1);
      // Bits:47-40 - core4
      MAX_TURBO_6C =
          get_msr_value(CPU_NUM, MSR_TURBO_RATIO_LIMIT, 47, 40, &error_indx);
      SET_IF_TRUE(error_indx, online_cpus[0], -1);
    }
    // fflush (stdout);
    // sleep (1);
    ...
    if (numCPUs <= 0) {
      sprintf(string_ptr1,
              "  Max TURBO Multiplier (if Enabled) with 0 cores is");
      sprintf(string_ptr2, " %dx/%dx ", MAX_TURBO_1C, MAX_TURBO_2C);
    }
    if (numCPUs >= 1 && numCPUs < 4) {
      sprintf(string_ptr1,
              "  Max TURBO Multiplier (if Enabled) with 1/2 Cores is");
      sprintf(string_ptr2, "  ");
    }
    if (numCPUs >= 2 && numCPUs < 6) {
      sprintf(string_ptr1,
              "  Max TURBO Multiplier (if Enabled) with 1/2/3/4 Cores is");
      sprintf(string_ptr2, " %dx/%dx/%dx/%dx ", MAX_TURBO_1C, MAX_TURBO_2C,
    ...
    if (numCPUs >= 2 && numCPUs >= 6) { // Gulftown 6-cores, Nehalem-EX
      ...
      sprintf(string_ptr2, " %dx/%dx/%dx/%dx/%dx/%dx ", MAX_TURBO_1C,
    ...
    printf("C1 = Processor running with halts (States >C0 are power saver "
           "modes with cores idling)");
    printf("C3 = Cores running with PLL turned off and core cache turned off");
    printf("C6, C7 = Everything in C3 + core state saved to last level cache, "
           "C7 is deeper than C6");
    ...
    printf("Socket [%d] - [physical cores=%d, logical cores=%d, max online "
           "cores ever=%d] \n",
    ...
    if (TURBO_MODE == 1) {
      ...
      printf("  Max Frequency without considering Turbo %0.2f MHz (%0.2f x "
             "[%d]) \n",
    ...
    // Primarily for 32-bit users, found that after sometimes the counters
    // loopback, so inorder to prevent loopback, reset the counters back to 0
    // after 10 iterations roughly 10 secs
    if (*kk_1 > 10) {
      *kk_1 = 0;
      for (i = 0; i < numCPUs; i++) {
        ...
        if (prog_options.i7_version.sandy_bridge ||
            prog_options.i7_version.ivy_bridge ||
            prog_options.i7_version.haswell) {
    ...
    nanosleep(&one_second_sleep, NULL);
    if (prog_options.i7_version.sandy_bridge ||
        prog_options.i7_version.ivy_bridge || prog_options.i7_version.haswell) {
      printf("\tCore [core-id]  :Actual Freq (Mult.)\t  C0%%   Halt(C1)%%  C3 "
             "%%   C6 %%   C7 %%  Temp      VCore\n");
    } else {
      printf("\tCore [core-id]  :Actual Freq (Mult.)\t  C0%%   Halt(C1)%%  C3 "
             "%%   C6 %%  Temp      VCore\n");
    ...
    for (i = 0; i < numCPUs; i++) {
      ...
      if (old_val_CORE[ii] > new_val_CORE[ii]) { // handle overflow
        CPU_CLK_UNHALTED_CORE =
            (UINT64_MAX - old_val_CORE[ii]) + new_val_CORE[ii];
      } else {
      ...
      if (old_val_REF[ii] > new_val_REF[ii]) { // handle overflow
        CPU_CLK_UNHALTED_REF = (UINT64_MAX - old_val_REF[ii]) +
                               new_val_REF[ii]; // 3.40282366921e38
      } else {
      ...
      _FREQ[ii] = THRESHOLD_BETWEEN_0_6000(estimated_mhz *
                                           ((long double)CPU_CLK_UNHALTED_CORE /
      ...
      C1_time[ii] =
          ((long double)CPU_CLK_C1 / (long double)(new_TSC[ii] - old_TSC[ii]));
      C3_time[ii] =
          ((long double)CPU_CLK_C3 / (long double)(new_TSC[ii] - old_TSC[ii]));
      C6_time[ii] =
          ((long double)CPU_CLK_C6 / (long double)(new_TSC[ii] - old_TSC[ii]));
    ...
    for (ii = 0; ii < numCPUs; ii++) {
      ...
      if (prog_options.i7_version.sandy_bridge ||
        ...
        if (!IS_THIS_BETWEEN_0_100(C0_time[i] * 100) ||
            !IS_THIS_BETWEEN_0_100(C1_time[i] * 100 -
                                   (C3_time[i] + C6_time[i]) * 100) ||
            !IS_THIS_BETWEEN_0_100(C3_time[i] * 100) ||
            !IS_THIS_BETWEEN_0_100(C6_time[i] * 100) ||
            !IS_THIS_BETWEEN_0_100(C7_time[i] * 100) || isinf(_FREQ[i]))
          print_core[ii] = 0;
        else
          print_core[ii] = 1;
      } else {
        if (!IS_THIS_BETWEEN_0_100(C0_time[i] * 100) ||
            !IS_THIS_BETWEEN_0_100(C1_time[i] * 100 -
                                   (C3_time[i] + C6_time[i]) * 100) ||
            !IS_THIS_BETWEEN_0_100(C3_time[i] * 100) ||
            !IS_THIS_BETWEEN_0_100(C6_time[i] * 100) || isinf(_FREQ[i]))
          print_core[ii] = 0;
        else
          print_core[ii] = 1;
      }
    ...
    // Now print the information about the cores. Print garbage values message
    // if there is garbage
    for (ii = 0; ii < numCPUs; ii++) {
      assert(ii < MAX_SK_PROCESSORS);
      i = core_list[ii];
      ...
      if (prog_options.i7_version.sandy_bridge ||
        ...
        // if this happens c1_time might be negative so just adjust so that it
        // is thresholded to 0
        c1_time =
            C1_time[i] * 100 - (C3_time[i] + C6_time[i] + C7_time[i]) * 100;
        ...
        if (print_core[ii])
          printf("\tCore %d [%d]:\t  %0.2f "
                 "(%.2fx)\t%4.3Lg\t%4.3Lg\t%4.3Lg\t%4.3Lg\t%4.3Lg\t%d\t%0.4f\n",
                 ii + 1, core_list[ii], _FREQ[i], _MULT[i],
                 ...
                 THRESHOLD_BETWEEN_0_100(C7_time[i] * 100),
                 Read_Thermal_Status_CPU(
                     core_list[ii]), // C0_time[i]*100+C1_time[i]*100 around 100
        ...
        if (print_core[ii])
          printf("\tCore %d [%d]:\t  %0.2f "
                 "(%.2fx)\t%4.3Lg\t%4.3Lg\t%4.3Lg\t%4.3Lg\t%d\t%0.4f\n",
                 ii + 1, core_list[ii], _FREQ[i], _MULT[i],
                 THRESHOLD_BETWEEN_0_100(C0_time[i] * 100),
                 THRESHOLD_BETWEEN_0_100(c1_time),
                 THRESHOLD_BETWEEN_0_100(C3_time[i] * 100),
    ...
    // I have space allocated for 6 cores to be printed per socket so from all
    // the present cores till 6 print a blank line
    ...
    for (ii = 0; ii < numCPUs; ii++) {
      ...
      logCpuCstates_single((float)THRESHOLD_BETWEEN_0_100(C0_time[i] * 100));
      logCpuCstates_single_c(",");
      c1_time = C1_time[i] * 100 - (C3_time[i] + C6_time[i] + C7_time[i]) * 100;
      logCpuCstates_single((float)THRESHOLD_BETWEEN_0_100(c1_time));
      logCpuCstates_single_c(",");
      logCpuCstates_single((float)THRESHOLD_BETWEEN_0_100(C3_time[i] * 100));
      logCpuCstates_single_c(",");
    ...
    memcpy(old_val_CORE, new_val_CORE, sizeof(*old_val_CORE) * numCPUs);
...
void print_i7z_single() {
  ...
  construct_socket_information(&chi, &socket_0, &socket_1, socket_0_num,
                               socket_1_num);
  //	  print_socket_information(&socket_0);
  ...
  // Make an array size max 8 (to accomdate Nehalem-EXEX -lol) to store the
  // core-num that are candidates for a given socket removing it from here as it
  // is already allocated in the function int *core_list, core_list_size_phy,
  // core_list_size_log;
  ...
  unsigned long long int old_val_CORE[2][numCPUs_max],
      new_val_CORE[2][numCPUs_max];
  unsigned long long int old_val_REF[2][numCPUs_max],
      new_val_REF[2][numCPUs_max];
  ...
  for (;;) {
    ...
    // kk_1 and kk_2 are the variables that have to be set, i have to use them
    // internally so in future if there are more sockets to be printed, add more
    // kk_*
    socket_num = 0;
    printw_offset = 0;
    ...
    // below code in (else case) is to handle when for 2 sockets system, cpu1 is
    // populated and cpu0 is empty. single socket code but in an intelligent
    // manner and not assuming that cpu0 is always populated before cpu1
    if (socket_0.max_cpu > 1) {
      socket_num = 0;
      print_i7z_socket_single(
          socket_0, printw_offset, PLATFORM_INFO_MSR, PLATFORM_INFO_MSR_high,
          PLATFORM_INFO_MSR_low, online_cpus, cpu_freq_cpuinfo,
          one_second_sleep, TURBO_MODE, HT_ON_str, &kk_1,
          old_val_CORE[socket_num], old_val_REF[socket_num],
          old_val_C3[socket_num], old_val_C6[socket_num],
          old_val_C7[socket_num], old_TSC[socket_num], estimated_mhz,
          new_val_CORE[socket_num], new_val_REF[socket_num],
          new_val_C3[socket_num], new_val_C6[socket_num],
          new_val_C7[socket_num], new_TSC[socket_num], _FREQ[socket_num],
          _MULT[socket_num], C0_time[socket_num], C1_time[socket_num],
          C3_time[socket_num], C6_time[socket_num], C7_time[socket_num],
          tvstart[socket_num], tvstop[socket_num], &max_cpus_observed);
    } else {
      socket_num = 1;
      print_i7z_socket_single(
          socket_1, printw_offset, PLATFORM_INFO_MSR, PLATFORM_INFO_MSR_high,
          PLATFORM_INFO_MSR_low, online_cpus, cpu_freq_cpuinfo,
          one_second_sleep, TURBO_MODE, HT_ON_str, &kk_1,
          old_val_CORE[socket_num], old_val_REF[socket_num],
          old_val_C3[socket_num], old_val_C6[socket_num],
...
reserved.
          //Bits:0-7  - core1
          int MAX_TURBO_1C = get_msr_value (CPU_NUM, MSR_TURBO_RATIO_LIMIT, 7,
0, &error_indx);
          //Bits:15-8 - core2
          int MAX_TURBO_2C = get_msr_value (CPU_NUM, MSR_TURBO_RATIO_LIMIT, 15,
8, &error_indx);
          //Bits:23-16 - core3
          int MAX_TURBO_3C = get_msr_value (CPU_NUM, MSR_TURBO_RATIO_LIMIT, 23,
16, &error_indx);
          //Bits:31-24 - core4
          int MAX_TURBO_4C = get_msr_value (CPU_NUM, MSR_TURBO_RATIO_LIMIT, 31,
24, &error_indx);

          //gulftown/Hexacore support
          //technically these should be the bits to get for core 5,6
          //Bits:39-32 - core4
          int MAX_TURBO_5C = get_msr_value (CPU_NUM, MSR_TURBO_RATIO_LIMIT, 39,
32, &error_indx);
          //Bits:47-40 - core4
          int MAX_TURBO_6C = get_msr_value (CPU_NUM, MSR_TURBO_RATIO_LIMIT, 47,
40, &error_indx);
...
-n 1 > /tmp/cpufreq.txt"); system
                ("grep \"core id\" /proc/cpuinfo |sort -|uniq -|wc -l >
/tmp/numPhysical.txt"); system
                ("grep \"processor\" /proc/cpuinfo |sort -|uniq -|wc -l >
/tmp/numLogical.txt");
          //At this step, /tmp/numPhysical contains number of physical cores in
machine and
          //                      /tmp/numPhysical contains number of logical
cores in machine


          //Open the parsed cpufreq file and obtain the cpufreq from
/proc/cpuinfo FILE *tmp_file; tmp_file = fopen ("/tmp/cpufreq.txt", "r"); char
tmp_str[30]; fgets (tmp_str, 30, tmp_file); double cpu_freq_cpuinfo = atof
(tmp_str); fclose (tmp_file);

          //Parse the numPhysical and numLogical file to obtain the number of
physical and logical core tmp_file = fopen ("/tmp/numPhysical.txt", "r"); fgets
(tmp_str, 30, tmp_file); numPhysicalCores = atoi (tmp_str); fclose (tmp_file);
          ...
          //Print a slew of information on the ncurses window
          printf ("Cpu speed from cpuinfo %0.2fMhz\n", cpu_freq_cpuinfo);
          printf ("cpuinfo might be wrong if cpufreq is enabled. To guess
correctly try estimating via tsc\n"); printf ("Linux's inbuilt cpu_khz code
emulated now\n\n");


          //estimate the freq using the estimate_MHz() code that is almost mhz
accurate cpu_freq_cpuinfo = estimate_MHz (); printf ("True Frequency (without
accounting Turbo) %0.0f MHz\n", cpu_freq_cpuinfo);
          ...
          //We just need one CPU (we use Core-1) to figure out the multiplier
and the bus clock freq.
          //multiplier doesnt automatically include turbo
          //note turbo is not guaranteed, only promised
          //So this msr will only reflect the actual multiplier, rest has to be
figured out CPU_NUM = 0; CPU_Multiplier = get_msr_value (CPU_NUM,
PLATFORM_INFO_MSR, PLATFORM_INFO_MSR_high, PLATFORM_INFO_MSR_low, &error_indx);
          SET_IF_TRUE(error_indx,online_cpus[0],-1);
...
\n", CPU_Multiplier, BLCK);
          ...
          //Get turbo mode status by reading msr within turbo_status
          TURBO_MODE = turbo_status ();
//get_msr_value(CPU_NUM,IA32_MISC_ENABLE, TURBO_FLAG_high,TURBO_FLAG_low);

          //to find how many cpus are enabled, we could have used sysconf but
that will just give the logical numbers
          //if HT is enabled then the threads of the same core have the same
C-state residency number so...
          //Its imperative to figure out the number of physical and number of
logical cores.
          ...
          if (TURBO_MODE == 1)
                {				// &&
(CPU_Multiplier+1)==MAX_TURBO_2C){ printf ("TURBO ENABLED on %d Cores, %s\n",
numPhysicalCores, HT_ON_str); TRUE_CPU_FREQ = BLCK * ((double) CPU_Multiplier +
1); printf ("True Frequency %0.2f MHz (%0.2f x [%d]) \n", TRUE_CPU_FREQ, BLCK,
CPU_Multiplier + 1);
                }
          else
                {
          ...
          if (numCPUs >= 2)
                 {printf ("  Max TURBO (if Enabled) with 1/2 Core  active %dx /
%dx\n", MAX_TURBO_1C, MAX_TURBO_2C);} if (numCPUs >= 4) {printf ("  Max TURBO
(if Enabled) with 3/4 Cores active %dx / %dx\n", MAX_TURBO_3C, MAX_TURBO_4C);}
          if (numCPUs >= 6)
                 {printf ("  Max TURBO (if Enabled) with 5/6 Cores active %dx /
%dx\n", MAX_TURBO_5C, MAX_TURBO_6C);}

          printf ("C0 = Processor running without halting");
          printf ("C1 = Processor running with halts (States >C0 are power
saver)"); printf ("C3 = Cores running with PLL turned off and core cache turned
off"); printf ("C6 = Everything in C3 + core state saved to last level cache");
          printf ("  Above values in table are in percentage over the last 1
sec"); printf (" Total Logical Cores: [%d], Total Physical Cores: [%d] \n",
                        numLogicalCores, numPhysicalCores);
          ...
          int IA32_PERF_GLOBAL_CTRL = 911;	//38F
          int IA32_PERF_GLOBAL_CTRL_Value =	get_msr_value (CPU_NUM,
IA32_PERF_GLOBAL_CTRL, 63, 0, &error_indx);
          SET_IF_TRUE(error_indx,online_cpus[0],-1);
          int IA32_FIXED_CTR_CTL = 909;	//38D
          int IA32_FIXED_CTR_CTL_Value = get_msr_value (CPU_NUM,
IA32_FIXED_CTR_CTL, 63, 0, &error_indx);
          SET_IF_TRUE(error_indx,online_cpus[0],-1);
...
&error_indx); SET_IF_TRUE(error_indx,online_cpus[0],-1); CPU_CLK_UNHALTED_REF =
get_msr_value (CPU_NUM, 779, 63, 0, &error_indx);
          SET_IF_TRUE(error_indx,online_cpus[0],-1);
...
&error_indx); SET_IF_TRUE(error_indx,online_cpus[0],-1); unsigned long int
IA32_APERF = get_msr_value (CPU_NUM, 232, 7, 0, &error_indx);
          SET_IF_TRUE(error_indx,online_cpus[0],-1);
          printf ("Wait...\n");
          nanosleep (&one_second_sleep, NULL);
          IA32_MPERF = get_msr_value (CPU_NUM, 231, 7, 0, &error_indx) -
IA32_MPERF; SET_IF_TRUE(error_indx,online_cpus[0],-1); IA32_APERF =
get_msr_value (CPU_NUM, 232, 7, 0, &error_indx) - IA32_APERF;
          SET_IF_TRUE(error_indx,online_cpus[0],-1);
          ...
          for (;;)
                {
                      SET_ONLINE_ARRAY_PLUS1(online_cpus)
                          if (kk > 10){
                                ...
                                for (ii = 0; ii < numCPUs; ii++)
                                {
                                  //Set up the performance counters and then
start reading from them CPU_NUM = ii; IA32_PERF_GLOBAL_CTRL_Value =
get_msr_value (CPU_NUM, IA32_PERF_GLOBAL_CTRL, 63, 0, &error_indx);
                                  SET_IF_TRUE(error_indx,online_cpus[ii],-1);
                                  CONTINUE_IF_TRUE(online_cpus[ii]==-1);

                                  set_msr_value (CPU_NUM, IA32_PERF_GLOBAL_CTRL,
0x700000003LLU);

                                  IA32_FIXED_CTR_CTL_Value = get_msr_value
(CPU_NUM, IA32_FIXED_CTR_CTL, 63, 0, &error_indx);
                                  SET_IF_TRUE(error_indx,online_cpus[ii],-1);
                                  CONTINUE_IF_TRUE(online_cpus[ii]==-1);

                                  set_msr_value (CPU_NUM, IA32_FIXED_CTR_CTL,
819);

                                  IA32_PERF_GLOBAL_CTRL_Value = get_msr_value
(CPU_NUM, IA32_PERF_GLOBAL_CTRL, 63, 0, &error_indx);
                                  SET_IF_TRUE(error_indx,online_cpus[ii],-1);
                                  CONTINUE_IF_TRUE(online_cpus[ii]==-1);

                  IA32_FIXED_CTR_CTL_Value = get_msr_value (CPU_NUM,
IA32_FIXED_CTR_CTL, 63, 0, &error_indx);
                                  SET_IF_TRUE(error_indx,online_cpus[ii],-1);
                                  CONTINUE_IF_TRUE(online_cpus[ii]==-1);

                                  old_val_CORE[ii] = get_msr_value (CPU_NUM,
778, 63, 0, &error_indx); SET_IF_TRUE(error_indx,online_cpus[ii],-1);
                                  CONTINUE_IF_TRUE(online_cpus[ii]==-1);

                                  old_val_REF[ii] = get_msr_value (CPU_NUM, 779,
63, 0, &error_indx); SET_IF_TRUE(error_indx,online_cpus[ii],-1);
                                  CONTINUE_IF_TRUE(online_cpus[ii]==-1);

                                  old_val_C3[ii] = get_msr_value (CPU_NUM, 1020,
63, 0, &error_indx); SET_IF_TRUE(error_indx,online_cpus[ii],-1);
                                  CONTINUE_IF_TRUE(online_cpus[ii]==-1);

                                  old_val_C6[ii] = get_msr_value (CPU_NUM, 1021,
63, 0, &error_indx); SET_IF_TRUE(error_indx,online_cpus[ii],-1);
                                  CONTINUE_IF_TRUE(online_cpus[ii]==-1);
                          ...
                          kk++;
                  nanosleep (&one_second_sleep, NULL);
                  printf ("\tProcessor  :Actual Freq (Mult.)  C0%%   Halt(C1)%%
C3 %%   C6 %%\n");
...
&error_indx); SET_IF_TRUE(error_indx,online_cpus[i],-1);
                          CONTINUE_IF_TRUE(online_cpus[i]==-1);
...
&error_indx); SET_IF_TRUE(error_indx,online_cpus[i],-1);
              ...
              new_TSC[i] = rdtsc ();
                          ...
                          {	  //handle overflow
                                  CPU_CLK_UNHALTED_CORE = (UINT64_MAX -
old_val_CORE[i]) + new_val_CORE[i]; }else{ CPU_CLK_UNHALTED_CORE =
new_val_CORE[i] - old_val_CORE[i];
                          }
                          ...
                          if ((new_TSC[i] - old_TSC[i]) < CPU_CLK_UNHALTED_CORE)
                                        ...
                                        {CPU_CLK_C1 = ((new_TSC[i] - old_TSC[i])
- CPU_CLK_UNHALTED_CORE);}

                          if (old_val_REF[i] > new_val_REF[i])
                          {   //handle overflow
                                  CPU_CLK_UNHALTED_REF = (UINT64_MAX -
old_val_REF[i]) + new_val_REF[i]; //3.40282366921e38 }else{ CPU_CLK_UNHALTED_REF
= new_val_REF[i] - old_val_REF[i];
                          }

                          if (old_val_C3[i] > new_val_C3[i])
                          {   //handle overflow
                                  CPU_CLK_C3 = (UINT64_MAX - old_val_C3[i]) +
new_val_C3[i]; }else{ CPU_CLK_C3 = new_val_C3[i] - old_val_C3[i];
                          }

                          if (old_val_C6[i] > new_val_C6[i])
                          {   //handle overflow
                                  CPU_CLK_C6 = (UINT64_MAX - old_val_C6[i]) +
new_val_C6[i]; }else{ CPU_CLK_C6 = new_val_C6[i] - old_val_C6[i];
                          }

                          _FREQ[i] =
                                estimated_mhz * ((long double)
CPU_CLK_UNHALTED_CORE / (long double) CPU_CLK_UNHALTED_REF); _MULT[i] = _FREQ[i]
/ BLCK;

                          C0_time[i] = ((long double) CPU_CLK_UNHALTED_REF /
                                                   (long double) (new_TSC[i] -
old_TSC[i])); C1_time[i] = ((long double) CPU_CLK_C1 / (long double) (new_TSC[i]
- old_TSC[i])); C3_time[i] = ((long double) CPU_CLK_C3 / (long double)
(new_TSC[i] - old_TSC[i])); C6_time[i] = ((long double) CPU_CLK_C6 / (long
double) (new_TSC[i] - old_TSC[i]));
...
{C6_time[i] = 0;}
                          ...
                          //printf("%lld  - %lld", CPU_CLK_C6, (new_TSC[i] -
old_TSC[i]));
                  }
                  ...
                  for (i = 0; i < numCPUs; i++){
                        if(online_cpus[i]==-1){
                                ...
                                printf ("\tProcessor %d:  %0.2f
(%.2fx)\t%4.3Lg\t%4.3Lg\t%4.3Lg\t%4.3Lg\n", i + 1, _FREQ[i], _MULT[i],
                                      C0_time[i] * 100 , C1_time[i]* 100  -
(C3_time[i] + C6_time[i] )* 100, C3_time[i]* 100 , C6_time[i]* 100);
//C0_time[i]*100+C1_time[i]*100 around 100
                        }
                  }
                  ...
                  for (i = 0; i < numCPUs; i++)
                  {
                          ...
                          {
                                  TRUE_CPU_FREQ = _FREQ[i];
                                  printf ("True Frequency %0.2f MHz (Intel
specifies largest of below to be running Freq)\n", TRUE_CPU_FREQ);
                          }
                  }
                  ...
                  memcpy (old_val_REF, new_val_REF, sizeof (unsigned long int) *
numCPUs); memcpy (old_val_C3, new_val_C3, sizeof (unsigned long int) * numCPUs);
                  memcpy (old_val_C6, new_val_C6, sizeof (unsigned long int) *
numCPUs); memcpy (tvstart, tvstop, sizeof (struct timeval) * numCPUs); memcpy
(old_TSC, new_TSC, sizeof (unsigned long long int) * numCPUs); }   //ENDOF
INFINITE FOR LOOP exit (0); return (1);
}*/
