/* This  file is modified from source available at
http://www.kernel.org/pub/linux/utils/cpu/msr-tools/ for Model specific cpu
registers Modified to take i7 into account by Abhishek Jaiantilal
...
int Read_Thermal_Status_CPU(int cpu_num) {
  int error_indx;
  unsigned long val =
      get_msr_value(cpu_num, IA32_THERM_STATUS, 63, 0, &error_indx);
...
float Read_Voltage_CPU(int cpu_num) {
  int error_indx;
  unsigned long val =
      get_msr_value(cpu_num, MSR_PERF_STATUS, 47, 32, &error_indx);
...
void print_family_info(struct family_info *proc_info) {
  ...
  //    int*)(&proc_info->extended_family)); printf("    Extended Family %d\n",
...
double estimate_MHz() {
  // copied blantantly from
  // http://www.cs.helsinki.fi/linux/linux-kernel/2001-37/0256.html
  /*
  ...
  /*1 sec will cause rdtsc to overlap multiple times perhaps. 100msecs is a good
   * spot */
  usleep(10000);
  ...
  // \n", mhz, elapsed, microseconds); printf("%g  elapsed %llu  microseconds
...
void Print_Information_Processor(bool *nehalem, bool *sandy_bridge,
  ...
  */
  // somehow using strcmp or strncmp is crashing the app when using -O2, -O3
  // with gcc-4.7 (strncmp (vendor_string, "GenuineIntel",12) == 0) (strcmp
  // (vendor_string, "GenuineIntel",12) == 0)

  if (strcmp(vendor_string, "GenuineIntel") == 0) {
    // if (equal_string) {
    printf("i7z DEBUG: Found Intel Processor\n");
  } else {
    printf("this was designed to be an intel proc utility. You can perhaps mod "
           "it for your machine?\n");
    exit(1);
  }
  ...
  if (proc_info.family >= 0x6) {
    if (proc_info.extended_model == 0x1) {
      ...
      *haswell = true;
      ...
      case 0x5:
        ...
        *ivy_bridge = false;
        *haswell = false;
        printf("i7z DEBUG: Detected an i3/i5/i7 - 32nm (westmere - 1st "
               "generation core)\n");
        break;
      case 0xD:
        *nehalem = false;
        ...
        *ivy_bridge = false;
        *haswell = false;
        printf("i7z DEBUG: Detected an i7 - 32nm (haven't seen this version "
               "around, do write to me with the model number)\n");
        break;
      }
    ...
    printf("i7z DEBUG: Unknown processor, not exactly based on Nehalem\n");
    printf("If you are using an AMD processor, I highly recommend "
           "TurionPowerControl http://code.google.com/p/turionpowercontrol/\n");
    exit(1);
  }
}
...
void Test_Or_Make_MSR_DEVICE_FILES() {
  ...
  if (access("/dev/cpu/0/msr", F_OK) == 0) {
    ...
    if (access("/dev/cpu/0/msr", W_OK) == 0) {
      ...
      printf(
          "i7z DEBUG: You DO NOT have write permissions to msr device files\n");
      printf("i7z DEBUG: A solution is to run this program as root\n");
      exit(1);
    }
  } else {
    printf("i7z DEBUG: msr device files DO NOT exist, trying out a makedev "
           "script\n");
    if (geteuid() == 0) {
      ...
      printf("i7z DEBUG: You DO NOT have root privileges, mknod to create "
             "device entries won't work out\n");
      printf("i7z DEBUG: A solution is to run this program as root\n");
      exit(1);
    }
  }
...
double cpufreq_info() {
  // CPUINFO is wrong for i7 but correct for the number of physical and logical
  // cores present If Hyperthreading is enabled then, multiple logical processors
  // will share a common CORE ID
  // http://www.redhat.com/magazine/022aug06/departments/tips_tricks/
  system("cat /proc/cpuinfo |grep MHz|sed 's/cpu\\sMHz\\s*:\\s//'|tail -n 1 > "
         "/tmp/cpufreq.txt");
...
void print_socket_information(struct cpu_socket_info *socket) {
  ...
  printf("Socket-%d [num of cpus %d physical %d logical %d] %s\n",
         socket->socket_num, socket->max_cpu, socket->num_physical_cores,
         socket->num_logical_cores, socket_list);
}
...
void print_CPU_Heirarchy(struct cpu_heirarchy_info chi) {
  ...
  for (i = 0; i < chi.max_online_cpu; i++) {
    ...
    printf("--[%d] Socket number/Hyperthreaded Sibling number  %d,%d\n", i,
           chi.package_num[i], chi.sibling_num[i]);
    printf("--[%d] Core id number %d\n", i, chi.coreid_num[i]);
    printf("--[%d] Display core in i7z Tool: %s\n\n", i,
           (chi.display_cores[i] == 1) ? "Yes" : "No");
  }
}
