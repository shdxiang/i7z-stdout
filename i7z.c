...
void logCpuFreq_single(float value) {
  ...
  if (prog_options.logging == 1) {
    fprintf(fp_log_file_freq, "%f\n",
            value); // newline, replace \n with \t to get everything separated
                    // with tabs
  }
...
void logCpuFreq_single_ts(
    struct timespec *value) // HW use timespec to avoid floating point overflow
{
  // below when just logging
  if (prog_options.logging == 1) {
    fprintf(fp_log_file_freq, "%d.%.9d\n", value->tv_sec,
            value->tv_nsec); // newline, replace \n with \t to get everything
                             // separated with tabs
  }
  // below when appending
  if (prog_options.logging == 2) {
...
void logCpuFreq_dual_ts(
    struct timespec *value,
    int socket_num) // HW use timespec to avoid floating point overflow
{
  if (socket_num == 0) {
    // below when just logging
    if (prog_options.logging == 1)
...
// fix for issue 48, suggested by Hakan
void logCpuCstates_single_ts(
    struct timespec *value) // HW use timespec to avoid floating point overflow
{
  // below when just logging
  if (prog_options.logging != 0) {
    fprintf(fp_log_file_Cstates, "%d.%.9d", value->tv_sec,
            value->tv_nsec); // newline, replace \n with \t to get everything
                             // separated with tabs
  }
}
...
void logCpuCstates_dual(float value, int socket_num) {
  if (socket_num == 0) {
    // below when just logging
    if (prog_options.logging != 0)
      fprintf(fp_log_file_Cstates_1, "%f",
              value); // newline, replace \n with \t to get everything separated
                      // with tabs
  }
  if (socket_num == 1) {
    // below when just logging
    if (prog_options.logging != 0)
      fprintf(fp_log_file_Cstates_2, "%f",
              value); // newline, replace \n with \t to get everything separated
                      // with tabs
  }
}
...
void logCpuCstates_dual_c(char *value, int socket_num) {
  if (socket_num == 0) {
    // below when just logging
    if (prog_options.logging != 0)
      fprintf(fp_log_file_Cstates_1, "%s",
              value); // newline, replace \n with \t to get everything separated
                      // with tabs
  }
  if (socket_num == 1) {
    // below when just logging
    if (prog_options.logging != 0)
      fprintf(fp_log_file_Cstates_2, "%s",
              value); // newline, replace \n with \t to get everything separated
                      // with tabs
  }
}
...
void logCpuCstates_dual_d(int value, int socket_num) {
  if (socket_num == 0) {
    // below when just logging
    if (prog_options.logging != 0)
      fprintf(fp_log_file_Cstates_1, "%d",
              value); // newline, replace \n with \t to get everything separated
                      // with tabs
  }
  if (socket_num == 1) {
    // below when just logging
    if (prog_options.logging != 0)
      fprintf(fp_log_file_Cstates_2, "%d",
              value); // newline, replace \n with \t to get everything separated
                      // with tabs
  }
}

void logCpuCstates_dual_ts(
    struct timespec *value,
    int socket_num) // HW use timespec to avoid floating point overflow
{
  if (socket_num == 0) {
    // below when just logging
    if (prog_options.logging != 0)
      fprintf(fp_log_file_Cstates_1, "%d.%.9d", value->tv_sec,
              value->tv_nsec); // newline, replace \n with \t to get everything
                               // separated with tabs
  }
  if (socket_num == 1) {
    // below when just logging
    if (prog_options.logging != 0)
      fprintf(fp_log_file_Cstates_2, "%d.%.9d", value->tv_sec,
              value->tv_nsec); // newline, replace \n with \t to get everything
                               // separated with tabs
  }
}
...
void modprobing_msr() { system("modprobe msr"); }
...
int main(int argc, char **argv) {
  ...
  char log_file_name[MAX_FILENAME_LENGTH],
      log_file_name2[MAX_FILENAME_LENGTH + 3];
  ...
  static struct option long_options[] = {{"write", required_argument, 0, 'w'},
  ...
  while (1) {
    ...
    case 'l':
      ...
      printf("Logging frequencies to %s for single sockets, %s for dual "
             ...
             CPU_FREQUENCY_LOGGING_FILE_single,
             CPU_FREQUENCY_LOGGING_FILE_dual);
      break;
    ...
    case 'h':
      ...
      printf(
          "Default log file name is %s (single socket) or %s (dual socket)\n",
      ...
      printf("%c[%dm", 0x1B, 0);
      printf("Specifying a particular socket to print: %c[%d;%d;%dm./i7z "
             "--socket0 X \n",
             0x1B, 1, 31, 40);
      printf("%c[%dm", 0x1B, 0);
      printf("In order to print to a second socket use: %c[%d;%d;%dm./i7z "
             "--socket1 X \n",
             0x1B, 1, 31, 40);
      printf("%c[%dm", 0x1B, 0);
      printf("To turn the ncurses GUI off use: %c[%d;%d;%dm./i7z --nogui\n",
             0x1B, 1, 31, 40);
      printf("%c[%dm", 0x1B, 0);
      printf("Example: To print for two sockets and also change the log file "
             "%c[%d;%d;%dm./i7z --socket0 0 --socket1 1 -logfile "
             "/tmp/logfilei7z -w l\n",
             0x1B, 1, 31, 40);
      printf("%c[%dm", 0x1B, 0);
  ...
  //	printf("nehalem %d, sandy brdige %d\n", prog_options.i7_version.nehalem,
  //prog_options.i7_version.sandy_bridge);
  ...
  */
  ...
  while( (c=getopt(argc,argv,"w:")) !=-1){
      ...
      //printf("argument %c\n",c);
      if(cvalue == NULL){
          printf("With -w option, requires an argument for append or
  logging\n"); exit(1); }else{
          //printf("         %s\n",cvalue);
          if(strcmp(cvalue,"a")==0){
              printf("Appending frequencies to %s (single_socket) or
  cpu_freq_log_dual_(%d/%d).txt (dual socket)\n",
  CPU_FREQUENCY_LOGGING_FILE_single,0,1); prog_options.logging=2; }else
  if(strcmp(cvalue,"l")==0){ printf("Logging frequencies to %s (single socket)
  or cpu_freq_log_dual_(%d/%d).txt (dual socket) \n",
  CPU_FREQUENCY_LOGGING_FILE_single,0,1); prog_options.logging=1; }else{
