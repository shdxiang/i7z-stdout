...
void print_i7z_socket(struct cpu_socket_info socket_0, int printw_offset,
                      int PLATFORM_INFO_MSR, int PLATFORM_INFO_MSR_high,
                      int PLATFORM_INFO_MSR_low, int *online_cpus,
                      double cpu_freq_cpuinfo, struct timespec one_second_sleep,
                      int *kk_1, U_L_L_I *old_val_CORE, U_L_L_I *old_val_REF,
                      U_L_L_I *old_val_C3, U_L_L_I *old_val_C6,
                      U_L_L_I *old_val_C7, U_L_L_I *old_TSC, int estimated_mhz,
                      U_L_L_I *new_val_CORE, U_L_L_I *new_val_REF,
                      U_L_L_I *new_val_C3, U_L_L_I *new_val_C6,
                      U_L_L_I *new_val_C7, U_L_L_I *new_TSC, double *_FREQ,
                      double *_MULT, long double *C0_time, long double *C1_time,
                      long double *C3_time, long double *C6_time,
                      long double *C7_time, struct timeval *tvstart,
  ...
  *max_observed_cpu = (socket_0.max_cpu > *max_observed_cpu)
  ...
  if (socket_0.max_cpu > 0) {
    ...
    for (ii = 0; ii < numCPUs; ii++) {
      ...
      if (prog_options.i7_version.sandy_bridge) {
        // there is a bit of leeway to be had as the counts might be off by a
        // bit thus threshold so that the diff is thresholded to 0
        c1_time =
    ...
    for (ii = 0; ii < numCPUs; ii++) {
      ...
      logCpuCstates_dual((float)THRESHOLD_BETWEEN_0_100(C0_time[i] * 100),
                         socket_0.socket_num);
      logCpuCstates_dual_c(",", socket_0.socket_num);
      // c1_time = C1_time[i] * 100 - (C3_time[i] + C6_time[i] + C7_time[i]) *
      // 100;
      c1_time = C1_time[i] * 100 - (C3_time[i] + C6_time[i]) * 100;
      logCpuCstates_dual((float)THRESHOLD_BETWEEN_0_100(c1_time),
                         socket_0.socket_num);
      logCpuCstates_dual_c(",", socket_0.socket_num);
      logCpuCstates_dual((float)THRESHOLD_BETWEEN_0_100(C3_time[i] * 100),
                         socket_0.socket_num);
      logCpuCstates_dual_c(",", socket_0.socket_num);
      logCpuCstates_dual((float)THRESHOLD_BETWEEN_0_100(C6_time[i] * 100),
                         socket_0.socket_num);
      if (prog_options.i7_version.sandy_bridge) {
        logCpuCstates_dual_c(",", socket_0.socket_num);
        logCpuCstates_dual((float)THRESHOLD_BETWEEN_0_100(C7_time[i] * 100),
                           socket_0.socket_num);
      }
      logCpuCstates_dual_c("]\t", socket_0.socket_num);
    }
...
void print_i7z() {
  ...
  print_i7z_socket(
      ...
      tvstart[socket_num], tvstop[socket_num], &max_observed_cpu_socket1);
  ...
  print_i7z_socket(
      ...
      PLATFORM_INFO_MSR_low, online_cpus, cpu_freq_cpuinfo, one_second_sleep,
      &kk_2, old_val_CORE[socket_num], old_val_REF[socket_num],
      old_val_C3[socket_num], old_val_C6[socket_num], old_val_C7[socket_num],
      old_TSC[socket_num], estimated_mhz, new_val_CORE[socket_num],
      new_val_REF[socket_num], new_val_C3[socket_num], new_val_C6[socket_num],
      new_val_C7[socket_num], new_TSC[socket_num], _FREQ[socket_num],
      _MULT[socket_num], C0_time[socket_num], C1_time[socket_num],
      C3_time[socket_num], C6_time[socket_num], C7_time[socket_num],
      tvstart[socket_num], tvstop[socket_num], &max_observed_cpu_socket2);
}
